...
- **Read** — to load `<config-root>/plugins/research.user-context.md` (passed by parent) and any cached candidate file from a prior run.

## Inputs

The parent skill passes:

- **`user-context-path`** (required) — path to the user's `<config-root>/plugins/research.user-context.md`. You read this first to learn topic, audience, sources, and ranking preferences.
